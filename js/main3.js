...
; (function (window) {
	...
	Day.prototype._setContentTitleFx = function (contentTitleEl) {
		// 1. Obtenemos el texto original del título.
		var originalText = contentTitleEl.textContent;
		// 2. Lo separamos en palabras, envolvemos cada palabra en un span, y lo unimos de nuevo.
		var wrappedText = originalText.split(' ').map(function (word) {
			return '<span class="word-wrapper">' + word + '</span>';
		}).join(' ');
		// 3. Reemplazamos el contenido del título con nuestro nuevo HTML.
		contentTitleEl.innerHTML = wrappedText;
	...
	function Calendar(el) {
		...
		this.el.parentNode.appendChild(this.dayPreview);
	...
	Calendar.prototype._showPreviewTitle = function (text, number) {
		this.dayPreview.innerHTML = `<span class="title__number"></span><span class="title__text">${text.trim()}</span>`;
	...
	Calendar.prototype._showContent = function (day) {
		...
		// Highlight Day 17 in black (special date)
		if (this.currentDayIdx === 16) {
			contentNumber.style.color = '#000000';
		} else {
			contentNumber.style.color = ''; // Reset to default CSS
		}
	...
	var calendarEl = document.querySelector('.calendar-wrap .calendar') || document.querySelector('.calendar'),
		...
		bgEl = document.body,
		defaultBgColor = bgEl.style.backgroundColor || '#E89AA7',
		colortimeout,
	...
	// Sistema de audio automático con fallback
